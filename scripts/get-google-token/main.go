...
func main() {
	...
	config, err := google.ConfigFromJSON(b, services.ListChatScopes()...)
	if err != nil {
	...
	_, err = srv.Spaces.List().Do()
	if err != nil {
		log.Fatalf("Unable to list spaces: %v", err)
	}

	tokenFileAbsPath, err := filepath.Abs(*tokenPath)
	if err != nil {
		log.Fatalf("Unable to get absolute path of token file: %v", err)
	}
